...
gaus_post <- function(preds, sig2chain, true_mean, region, popsize) {
  ...
  for (i in seq_along(regions)) {
    row_idx[idx_by_reg[[regions[i]]]] <- i
  }
  ...
  if (!is.null(names(true_mean))) {
    true_mean <- true_mean[regions]
  }
  ...
  est_nonoise <- rowMeans(post_mean)
  sigma2_nonoise <- apply(post_mean, 1, stats::var)
  lb_nonoise <- apply(post_mean, 1, stats::quantile, probs = 0.025)
  ub_nonoise <- apply(post_mean, 1, stats::quantile, probs = 0.975)
  cr_nonoise <- mean(lb_nonoise <= true_mean & true_mean <= ub_nonoise)
  ...
  list(
    ...
    sigma2 = sigma2,
    post = post,
    est_nonoise = est_nonoise,
    lb_nonoise = lb_nonoise,
    ub_nonoise = ub_nonoise,
    cr_nonoise = cr_nonoise,
    sigma2_nonoise = sigma2_nonoise,
...
MTSM_br <- function(X_1, X_2, Z_1, Z_2, S, sig2b = 1000, wgt = NULL, n = NULL,
  ...
  for (index in seq_len(nsim + nburn)) {
    ...
    Q_eta <- tau_1^2 * (S_w_S / sig2) + Ir / sig2e + tau_2^2 * OM
    ...
    b_Beta_2 <- tX2_k - tau_2 * t(S_w_X2) %*% eta - t(S_w_X2) %*% lambda
    Beta_2 <- rmvn_prec(b_Beta_2, Q_Beta_2)

    var_tau_1 <- 1 / (sum(eta * ((S_w_S / sig2) %*% eta)) + 1 / sig2t)
...
unis_bios_grouped <- function(X, y_sum, n_sum, S, sig2b = 1000,
                              predX, predS,
                              nburn = 1000, nsim = 5000, nthin = 1,
                              a = 0.1, b = 0.1) {
  N <- nrow(X)
...
MTSM_br_grouped <- function(X_1, Z_1, S_1, wgt_1,
  ...
  for (index in seq_len(nsim + nburn)) {
    ...
    Q_eta <- tau_1^2 * (S1_w_S1 / sig2) + Ir / sig2e + tau_2^2 * OM
    ...
    Q_Beta_2 <- X2_w_X2 + Ip2 / sig2b
    b_Beta_2 <- tX2_k - tau_2 * t(S2_w_X2) %*% eta - t(S2_w_X2) %*% lambda
    Beta_2 <- rmvn_prec(b_Beta_2, Q_Beta_2)

    var_tau_1 <- 1 / (sum(eta * ((S1_w_S1 / sig2) %*% eta)) + 1 / sig2t)
